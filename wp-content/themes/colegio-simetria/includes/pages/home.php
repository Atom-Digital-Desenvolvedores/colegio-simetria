<?php
	...
	function metaboxes_home() {
		...
		$banner->add_group_field( $banner_items, array(
			...
			'desc'       => __( 'Tamanho recomendado <strong>435x435</strong>' ),
			'id'         => 'wsg_banner_items_img',
			'type' => 'file',
			'preview_size' => array( 435/2, 435/2 ),
			'query_args' => array( 'type' => 'image' ),
		) );

		$banner->add_group_field( $banner_items, array(
			'name'       => __( 'Titulo do botão de selecionar o banner' ),
			'id'         => 'wsg_banner_items_select_titulo',
			'type'       => 'text',
		) );

		$banner->add_group_field( $banner_items, array(
			'name'       => __( 'Cor de fundo desse banner' ),
			'id'         => 'wsg_banner_items_colorpicker',
			'type'       => 'colorpicker',
			'default'	 => '#0000ff'
		) );
		$banner->add_group_field( $banner_items, array(
